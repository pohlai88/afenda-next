 ...
 * @afenda-owner repo
 * @afenda-subject env
 * @afenda-artifact sync
 * @afenda-boundary automation
 * @afenda-description Automation sync for canonical local environment file generation
 */
...
if (!existsSync(source)) {
  console.error(
    "Missing .env.config at repository root. Run this from the project root.",
  );
  process.exit(1);
}

const sourceContent = readFileSync(source, "utf8");
const targetContent = appendMissingCanonicalBetterAuthValues(sourceContent);
...
function appendMissingCanonicalBetterAuthValues(content) {
  ...
  return `${trimTrailingNewlines(content)}\n\n# Canonical Better Auth values normalized by pnpm env:sync\n${lines.join("\n")}\n`;
}
