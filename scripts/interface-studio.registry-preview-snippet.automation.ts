 ...
 * @afenda-description Preview module bodies for Interface Studio registry codegen (run from scripts only).
 *
...
function genericAriaLabeled(m: ApprovedComponentManifest): RegistryPreviewSnippet {
  return {
    ...
    body: `<${m.exportName} aria-label="Interface Studio preview" />`,
  };
}
...
function genericWithChildText(m: ApprovedComponentManifest, text: string): RegistryPreviewSnippet {
  return {
    importLines: [`import { ${m.exportName} } from "${m.sourcePath}";`],
    body: `<${m.exportName} aria-label="Interface Studio preview">${text}</${m.exportName}>`,
  };
}
  ...
  switch (m.id) {
    ...
    case "app-link":
      return {
        ...
        body: `<AppLink href="/interface-studio">Interface Studio</AppLink>`,
      };
    case "app-text-field":
      return {
        importLines: [
    ...
    case "app-breadcrumbs":
      return {
        ...
        body: `<AppBreadcrumbs><AppBreadcrumb href="/interface-studio">Lab</AppBreadcrumb><AppBreadcrumb>Preview</AppBreadcrumb></AppBreadcrumbs>`,
      };
    case "app-disclosure":
      return {
