...
```jsx
<ListBoxItem className="selected:bg-blue-400 data-disabled:bg-gray-100">
  Item
</ListBoxItem>
```
...
<Radio
  className={({ isFocusVisible, isSelected }) =>
    `flex rounded-lg p-4 ${isFocusVisible ? "ring-2 ring-blue-600 ring-offset-1" : ""} ${isSelected ? "border-white/30 bg-blue-600 text-white" : ""} `
  }
>
  {/* ... */}
</Radio>
