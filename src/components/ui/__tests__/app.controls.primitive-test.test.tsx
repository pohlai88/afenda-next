...
import {
  renderWithProviders,
  setupUser,
} from "@/test-runtime/test-runtime.render.helper.test";
...
describe("shared React Aria controls", () => {
  ...
  it("renders tabs, grid list selection, and toolbar semantics through shared wrappers", async () => {
    const user = setupUser();

    function TabsAndGridHarness() {
      const [selectedTab, setSelectedTab] = useState("overview");
      const [selectedItem, setSelectedItem] = useState("contracts");

      return (
        <div className="space-y-4">
          <AppToolbar ariaLabel="Queue filters">
          ...
          >
            <AppTabList aria-label="Workbench modes">
              <AppTab id="overview">Overview</AppTab>
              <AppTab id="contracts">Contracts</AppTab>
            </AppTabList>
            <AppTabPanels>
              <AppTabPanel id="overview">Overview panel</AppTabPanel>
              <AppTabPanel id="contracts">Contracts panel</AppTabPanel>
            </AppTabPanels>
          </AppTabs>

          <AppGridList
            aria-label="Contract items"
            onSelectionChange={(keys) => {
              if (keys === "all") {
                return;
              }
              ...
              if (firstKey !== undefined) {
                setSelectedItem(String(firstKey));
              }
            }}
          ...
          >
            <AppGridListItem id="contracts" textValue="Contracts">
              Contracts
            </AppGridListItem>
            <AppGridListItem id="methods" textValue="Methods">
              Methods
            </AppGridListItem>
          </AppGridList>
    ...
    renderWithProviders(<TabsAndGridHarness />);

    expect(
      screen.getByRole("toolbar", { name: "Queue filters" }),
    ).toBeInTheDocument();
    expect(screen.getByText("Selected tab: overview")).toBeInTheDocument();

    await user.click(screen.getByRole("tab", { name: "Contracts" }));
    expect(screen.getByText("Selected tab: contracts")).toBeInTheDocument();
    expect(screen.getByText("Contracts panel")).toBeInTheDocument();

    await user.click(screen.getByText("Methods"));
    expect(screen.getByText("Selected item: methods")).toBeInTheDocument();
  });
