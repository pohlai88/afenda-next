...
type AppTabsProps = Omit<TabsProps, "className"> & {
  children: React.ReactNode;
  className?: string;
};

type AppTabListProps<T extends object> = Omit<TabListProps<T>, "className"> & {
  children: React.ReactNode;
  className?: string;
};

type AppTabProps = Omit<TabProps, "className"> & {
  children: React.ReactNode;
  className?: string;
};
...
type AppTabPanelProps = Omit<TabPanelProps, "className"> & {
  children: React.ReactNode;
  className?: string;
};

type AppGridListProps<T extends object> = Omit<
  GridListProps<T>,
  "className"
> & {
  children: React.ReactNode;
...
type AppGridListItemProps = Omit<GridListItemProps, "className"> & {
  children: React.ReactNode;
  className?: string;
};
...
export function AppTabs({ children, className = "", ...props }: AppTabsProps) {
  return (
    <Tabs className={cn("flex min-h-0 flex-col gap-4", className)} {...props}>
      {children}
    </Tabs>
  );
}

export function AppTabList<T extends object>({
  children,
  className = "",
  ...props
}: AppTabListProps<T>) {
  return (
    <TabList
      className={cn(
        "border-border bg-surface-muted flex flex-wrap items-center gap-2 rounded-(--radius-panel) border p-2 outline-none",
        className,
      )}
      {...props}
    >
...
export function AppTab({ children, className = "", ...props }: AppTabProps) {
  return (
    <Tab
      className={({ isDisabled, isSelected }) =>
        cn(
          "rac-focus-ring type-label inline-flex min-h-[var(--control-height-compact)] items-center justify-center rounded-(--radius-control) border px-4 py-2.5 transition outline-none",
          isSelected
            ? "border-accent-strong bg-accent-soft text-foreground"
            : "text-foreground-muted hover:border-border-strong hover:bg-field-hover hover:text-foreground border-transparent bg-transparent",
          isDisabled && "rac-disabled",
          className,
        )
      }
      {...props}
...
export function AppTabPanels({ children, className = "" }: AppTabPanelsProps) {
  return <TabPanels className={cn("min-h-0", className)}>{children}</TabPanels>;
}
...
export function AppTabPanel({
  ...
  return (
    <TabPanel className={cn("min-h-0 outline-none", className)} {...props}>
      {children}
    </TabPanel>
  );
}

export function AppGridList<T extends object>({
  children,
  className = "",
  ...props
}: AppGridListProps<T>) {
  return (
    <GridList className={cn("grid gap-2 outline-none", className)} {...props}>
      {children}
    </GridList>
  );
}

export function AppGridListItem({
  children,
  className = "",
  ...props
}: AppGridListItemProps) {
  return (
    <GridListItem
      className={({ isFocused, isSelected }) =>
        cn(
          "rac-focus-ring border-border bg-surface hover:bg-surface-muted cursor-default rounded-(--radius-panel) border p-3 text-left transition outline-none",
          isSelected && "border-accent-strong bg-accent-soft",
          isFocused &&
            "ring-accent-ring ring-offset-background ring-2 ring-offset-2",
          className,
        )
      }
      {...props}
...
>) {
  return (
    <Toolbar
      ...
      className={cn(toolbarVariants({ density, tone }), className)}
      {...props}
    >
      {children}
    </Toolbar>
