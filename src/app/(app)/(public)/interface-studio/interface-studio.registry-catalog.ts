 ...
 * @afenda-description Maps ui governance manifests to Interface Studio catalog rows (server-only).
 */
...
import type { InterfaceStudioItem, InterfaceStudioStatus } from "./interface-studio.types";

function manifestStatusToLabStatus(status: ManifestLifecycleStatus): InterfaceStudioStatus {
  switch (status) {
    case "draft":
      return "experimental";
    case "review":
...
export function buildRegistryDerivedInterfaceStudioItems(): InterfaceStudioItem[] {
