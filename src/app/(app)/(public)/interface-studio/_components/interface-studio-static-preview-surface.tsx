...
type InterfaceStudioStaticPreviewSurfaceProps = {
  item: InterfaceStudioItem;
  previewKindLabel: string;
};

export function InterfaceStudioStaticPreviewSurface({
  item,
  previewKindLabel,
}: InterfaceStudioStaticPreviewSurfaceProps) {
    ...
    <div className="space-y-4">
      <div className="rounded-(--radius-panel) border border-border bg-background/80 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <span className={getInterfaceStudioStatusBadgeClassName(item.status)}>{item.status}</span>
        </div>
      </div>
