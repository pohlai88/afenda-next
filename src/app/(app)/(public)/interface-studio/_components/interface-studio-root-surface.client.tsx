 ...
 * @afenda-description Lab home: five-zone void shell with catalog routing; catalog sections keep InterfaceStudioShell.
 */
"use client";
...
import { INTERFACE_STUDIO_SECTIONS, INTERFACE_STUDIO_TITLE } from "../interface-studio.config";
import type { InterfaceStudioRouteSurface } from "../interface-studio.routes.shared";
import {
  buildInterfaceStudioSectionNavHref,
  buildInterfaceStudioStudioHomeHref,
} from "../interface-studio.routes.shared";

export type InterfaceStudioRootSurfaceProps = {
  studioSummary: {
    total: number;
    approved: number;
    candidate: number;
...
export function InterfaceStudioRootSurface({
  studioSummary,
  routeSurface = "studio",
}: InterfaceStudioRootSurfaceProps) {
  const pathname = usePathname() ?? "";
  const router = useRouter();
  const homeHref = buildInterfaceStudioStudioHomeHref(routeSurface);

  const navGroups = useMemo((): AppShellInterfaceNavGroup[] => {
    const sections = INTERFACE_STUDIO_SECTIONS;
    ...
    if (landing !== undefined) {
      const landingHref = buildInterfaceStudioSectionNavHref(routeSurface, landing.id);
      screens.push({
        id: landing.id,
        label: landing.title,
        href: landingHref,
    ...
    if (dashboard !== undefined) {
      const dashboardHref = buildInterfaceStudioSectionNavHref(routeSurface, dashboard.id);
      screens.push({
        id: dashboard.id,
    ...
    if (components !== undefined) {
      const componentsHref = buildInterfaceStudioSectionNavHref(routeSurface, components.id);
      groups.push({
        id: "primitives",
        label: "Primitives",
        items: [
    ...
    if (blocks !== undefined) {
      const blocksHref = buildInterfaceStudioSectionNavHref(routeSurface, blocks.id);
      groups.push({
        id: "blocks",
        label: "Blocks",
        items: [
    ...
    if (erp !== undefined) {
      const erpHref = buildInterfaceStudioSectionNavHref(routeSurface, erp.id);
      groups.push({
        id: "erp-patterns",
        label: "ERP patterns",
        items: [
  ...
  const commandItems = useMemo(
    () =>
      INTERFACE_STUDIO_SECTIONS.flatMap((section) => {
        const href = buildInterfaceStudioSectionNavHref(routeSurface, section.id);
        return [
          {
            id: `go-${section.id}`,
  ...
  return (
    <AppShellInterface
      labTitle={INTERFACE_STUDIO_TITLE}
      navGroups={navGroups}
      ...
      inspectorSchema={
        <div className="space-y-3 text-sm leading-6 text-zinc-400">
          <p>
            ...
            catalogs and detail previews use <code className="text-zinc-300">InterfaceStudioShell</code> on their own
            URLs.
          </p>
    ...
    >
      <div className="max-w-md px-4 text-center">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-2">
          {INTERFACE_STUDIO_SECTIONS.map((section) => (
            <Link
              key={section.id}
              href={buildInterfaceStudioSectionNavHref(routeSurface, section.id)}
