...
export default async function InterfaceLabErpPatternDetailPage({ params }: Props) {
  const { slug } = await params;
  const item = getCachedInterfaceLabItem(sectionId, slug);
  ...
  return (
    <InterfaceLabShell>
      <InterfaceLabShell.Header
        eyebrow="INTERFACE STUDIO / PATTERN"
        title={item.title}
        ...
        meta={
          <dl className="grid gap-3 sm:grid-cols-3">
            <div className="space-y-1">
              ...
              <dd className="text-sm font-medium text-foreground">Patterns</dd>
            </div>
            <div className="space-y-1">
              <dt className="text-xs font-medium uppercase tracking-[0.16em] text-foreground-muted">
                Category
            ...
            <div className="space-y-1">
              ...
              <dd className="text-sm font-medium text-foreground">{item.slug}</dd>
            </div>
          </dl>
        }
      />
        ...
        previewBody={
          <InterfaceLabComponentDetailWorkspace
            ...
            previewDescription={
              ...
              "Pattern artboard for behavior, state, and exception variants."
            }
            previewSurface={
              <InterfaceLabStaticPreviewSurface item={item} previewKindLabel="Behavior pattern" />
            }
