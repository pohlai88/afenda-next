...
import { AccountAreaNav } from "@/app/(app)/iam/account/_components/account-area-nav";
...
export const metadata: Metadata = {
  title: "Workspace",
  description: "Create tenants and manage your Afenda memberships.",
  robots: { index: false, follow: false },
};

export default async function AccountWorkspacePage() {
  const session = await requireSession("/iam/account/workspace");
    ...
    <main className="bg-background text-foreground min-h-screen">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-16">
              ...
              {memberships.length > 0 ? (
                memberships.map((membership) => (
                  <Link
                    ...
                    href={`/iam/t/${membership.tenant.slug}` as `/iam/t/${string}`}
        ...
        <div className="flex flex-wrap gap-3">
          <Link className="type-body-sm text-accent-strong hover:underline" href="/iam/account/select-tenant">
            Switch tenant
          </Link>
          <Link className="type-body-sm text-accent-strong hover:underline" href="/iam/auth/post-login">
