...
export const metadata: Metadata = {
  title: "Select tenant",
  description: "Choose which Afenda tenant to open.",
  robots: { index: false, follow: false },
};

export default async function AccountSelectTenantPage() {
  const session = await requireSession("/iam/account/select-tenant");
  const memberships = await listUserTenantMemberships(session.user.id);

  if (memberships.length === 0) {
    redirect("/iam/auth/post-login");
  }

  if (memberships.length === 1) {
    redirect(`/iam/t/${memberships[0]?.tenant.slug}` as `/iam/t/${string}`);
  }
  ...
  return (
    <main className="bg-background text-foreground min-h-screen">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 px-6 py-16">
        ...
        <Link className="type-body-sm text-accent-strong hover:underline" href="/iam/account/workspace">
          Manage tenants
        </Link>
      </div>
    </main>
