...
export function AccountAreaNav({ current }: Props) {
  return (
    <nav className="border-border bg-surface flex flex-wrap gap-2 rounded-(--radius-panel) border p-2">
      <NavLink current={current === "security"} href="/iam/account/security">
        Security
      </NavLink>
      <NavLink current={current === "identity"} href="/iam/account/identity">
        Identity
      </NavLink>
      <NavLink current={current === "workspace"} href="/iam/account/workspace">
        Workspace
      </NavLink>
    </nav>
  );
